...
if [ ! -f /home/$user/Renungan/$hari'.mp3' ]; then
    ...
    get_link_renungan=`curl -s dailyfreshjuice.net | grep -Po '(href=")[^"]*' | grep 'http://media.blubrry.com/dailyfreshjuice/p/dailyfreshjuice.net/wp-content/uploads/'$bulan'/FJ'$hari'' | sort -u | sed 's/\href=//g' | tr -d '"'`
