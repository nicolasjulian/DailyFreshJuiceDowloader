...
tahun=`date '+%Y'`
	...
	if [[ ! -d /home/$user/Renungan/ ]]; then
		...
		echo  "File Renungan Exist Now Start Downloading"
	fi

    		#Downloading file rennungan
		if [[ ! -f /home/$user/Renungan/$hari'.mp3' ]]; then
		    echo "Download Renungan Hari ini"
		    get_link_renungan=`curl  dailyfreshjuice.net | grep -Po '(href=")[^"]*' | grep 'http://media.blubrry.com/dailyfreshjuice/p/dailyfreshjuice.net/wp-content/uploads/'$tahun'/' | grep 'FJ'$hari'' | sort -u | sed 's/\href=//g' | tr -d '"'`
